...
const isProductHasError = ({
  ...
  if (!category || !(ObjectId.isValid(id) && String(new ObjectId(id)) === id)) {
  ...
  if (
    ...
    error.push("available sizes should be an array of object, size with price");
  }
...
export async function POST(req) {
  try {
    ...
    const userDetails = getTokenDetails(token.value);
